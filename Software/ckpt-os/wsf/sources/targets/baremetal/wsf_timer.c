...
wsfTimer_t *WsfTimerServiceExpired(wsfTaskId_t taskId)
{
  ...
  #ifdef CHECKPOINT
  pElem = WsfAppTimerServiceExpired(0);
  if (pElem != NULL) {
    return pElem;
  }
...
void WsfTimerSleep(void)
{
  ...
  if (!net_restored) {
    ...
    wsfTimerTicks_t nextAppExpiration = wsfAppTimerNextExpiration();
    nextExpiration = wsfTimerNextExpiration();
    if (nextExpiration == 0 || nextExpiration > nextAppExpiration) {
      WsfAppTimerSleep();
      return;
    }
  }
