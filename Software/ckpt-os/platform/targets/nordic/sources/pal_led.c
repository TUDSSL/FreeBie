...
void PalLedInit(void)
{
  ...
  nrf_gpio_cfg_output(DEBUG_INFO_CKPT);
  nrf_gpio_cfg_output(DEBUG_INFO_BLE);
  nrf_gpio_cfg_output(DEBUG_INFO_APP);
  #ifndef LCD_PINS_OCCUPIED
  nrf_gpio_pin_clear(DEBUG_INFO_FW_CONTENT);
  nrf_gpio_pin_clear(DEBUG_INFO_NET_CKPT);
  nrf_gpio_pin_clear(DEBUG_INFO_APP_CKPT);
  nrf_gpio_cfg_output(DEBUG_INFO_FW_CONTENT);
  nrf_gpio_cfg_output(DEBUG_INFO_NET_CKPT);
  nrf_gpio_cfg_output(DEBUG_INFO_APP_CKPT);
  #endif
#endif
#endif
 ...
 */
/*************************************************************************************************/
void PalLedToggle(uint8_t ledId)
{
#if defined(BOARD_PCA10056)
  switch (ledId)
  {
  // case PAL_LED_ID_CPU_ACTIVE:
  //   nrfx_gpiote_out_toggle(LED_2);
  //   break;
  // case PAL_LED_ID_ERROR:
  //   nrfx_gpiote_out_toggle(LED_4);
  //   break;
  // case 0:
  //   nrfx_gpiote_out_toggle(LED_1);
  //   break;
  // case 1:
