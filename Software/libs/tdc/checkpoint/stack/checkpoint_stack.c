...
size_t checkpoint_stack(void) {
  ...
  write_nvm(&cp->stack_pointer, &stack_ptr, sizeof(stack_ptr)); // todo might be trouble with the errata
...
size_t restore_stack(void) {
  ...
  char* stack_ptr = cp->stack_pointer; // todo might be trouble with the errata
  size_t size = stack_size(stack_ptr);
