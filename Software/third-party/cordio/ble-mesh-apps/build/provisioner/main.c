 ...
 *  \file
 *
 *  \brief  Main file for Mesh Provisioner application.
 *
...
extern void StackInitCfgProvisioner(void);

/*! \brief  Stack initialization for application */
extern void StackInitProvisioner(void);
...
int main(void)
{
  ...
  StackInitCfgProvisioner();

  /* Initialize console UI */
  WsfBufIoUartRegister(UiProcessUserInput);
  ProvisionerUiInit();

  /* Initialize the Mesh App Task (handler id installation). */
  StackInitProvisioner();

  /* Initialize Mesh Stack. */
  memUsed = MeshInit(WsfHeapGetFreeStartAddress(), WsfHeapCountAvailable());
  WsfHeapAlloc(memUsed);

  /* Initialize Mesh Configuration Client. */
  memUsed = MeshCfgMdlClInit(WsfHeapGetFreeStartAddress(), WsfHeapCountAvailable());
  WsfHeapAlloc(memUsed);
