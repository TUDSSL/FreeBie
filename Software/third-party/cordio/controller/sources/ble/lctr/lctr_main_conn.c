...
// based on third-party\cordio\controller\sources\ble\lctr\lctr_isr_conn_slave.c
// void lctrSlvConnEndOp(BbOpDesc_t *pOp);
void lctrSlvConnRecover(BbOpDesc_t *pOp, uint32_t timeFromMissedEventUsec)
{
  ...
  uint16_t numSkipCe = timeFromMissedEventUsec / (uint32_t)LCTR_CONN_IND_US(pCtx->connInterval);
  ...
  // {
    ...
    // uint32_t expTimeUsec = schGetTimeToExecBod(pOp);
...
//   return result;
// }
    uint32_t expTimeUsec = BbGetTargetTimeDelta(pOp->dueUsec, PalBbGetCurrentTime());
    if (expTimeUsec >= 300) {
      expTimeUsec -= 300;
    } else {
      expTimeUsec = 0;
    }
    uint32_t startTimeTick = ( PalRtcCounterGet() + BB_US_TO_BB_TICKS(expTimeUsec) ) & PAL_MAX_RTC_COUNTER_VAL;

    /* Set compare value to start BB. */
    PalRtcCompareSet(3, startTimeTick);
    /* Enable RTC interrupt source to start BB.  */
    PalRtcEnableCompareIrq(3);

    /* Set compare value to start HFCLK. */

    uint32_t startHFCLKTick = (startTimeTick - PAL_HFCLK_OSC_SETTLE_TICKS) & PAL_MAX_RTC_COUNTER_VAL;

    PalRtcClearCompareEvents(2);
    PalRtcCompareSet(2, startHFCLKTick);
  // }
}
