 ...
 */
/*************************************************************************************************/
void appConnUpdateTimerStart(dmConnId_t connId)
{
...
static void appSlaveConnUpdate(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  if (pAppUpdateCfg->idlePeriod != 0)
  {
    ...
    {
      #if defined(CHECKPOINT) || defined(COMPARE_NO_CHECKPOINT)
      // if I don't like the updated connection parameters
      if (pMsg->connUpdate.connInterval < pAppUpdateCfg->connIntervalMin) {
...
static void appSlaveRemoteConnParamReq(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  ...
  if ((pAppSlaveReqActCfg->remConnParamReqAct == APP_ACT_ACCEPT) && (pMsg->remConnParamReq.intervalMin >= 200))
  {
  ...
  else
  {
    ...
    DmRemoteConnParamReqNegReply(pCb->connId, HCI_ERR_CONN_INTERVAL);
  }
}
#else
static void appSlaveRemoteConnParamReq(dmEvt_t *pMsg, appConnCb_t *pCb)
